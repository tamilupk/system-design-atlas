...
export function ScalingStep({ onConceptClick }: StepComponentProps) {
  return <StepContent>
    <StepSection title="Scale sockets and conversation shards separately">
      ...
      <Paragraph>The load-balancer box represents redundant ingress, not one machine. It proxies both directions of each established WebSocket; it does not choose a conversation owner for each message. Sender and recipient devices establish separate connections, and the gateway box represents a fleet. The diagram shows delivery on the recipient’s existing socket; its handshake and receipts are omitted.</Paragraph>
      <Paragraph>Configure WebSocket upgrades, edge TLS and backend encryption, health checks, idle/connection timeouts, and draining. Heartbeats must fit the chosen timeout policy. Proxy or gateway loss breaks sockets: reconnect with jitter, re-authenticate, and recover by cursor. Track ingress connection count, handshake rate, bandwidth, and failure-domain capacity independently of gateway capacity; a load balancer does not automatically balance by application memory or outbound bytes.</Paragraph>
